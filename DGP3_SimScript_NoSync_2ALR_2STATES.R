...
fitSYM_2ALR_TS_2STATES <- function(X){
  ...
  trans <- function(teta){
    ...
    th[6] <- teta[6]^2
    th[7] <- teta[7]^2
    th[8]<- teta[8]^2/(1+teta[8]^2)
    th[9]<- teta[9]^2/(1+teta[9]^2)
    return(th)
  ...
  trans2<- function(teta){
    ...
    th[6] <- teta[6]^2
    th[7] <- teta[7]^2
    th[8]<- exp(teta[8])/(1+exp(teta[8]))
    th[9]<- exp(teta[9])/(1+exp(teta[9]))
    return(th)
    
  }
  
  BC<-X$State # vector of states
  AYc <-cbind(X$alr1,X$alr2)
  T<-length(AYc)
  captst<-nrow(AYc)
  prob0<-rep(0,captst)
  prob1<-rep(0,captst)
  ...
  NBER<-BC
  state0<-which(diff(NBER)==0)
  state1<-which(diff(NBER)==1)
  
  mu0<-colMeans(AYc[state0,])
  mu1<-colMeans(AYc[state1,])
  ...
  cova<-0.5;v2<-1;v3<-1
  p_00 <- 1;p_11<-0.5
  startval<-c(mu10,mu20,mu11,mu21,cova,v2,v3,p_00,p_11)
  ...
  res <- as.data.frame(cbind(State=BC,prob0,prob1))
...
distortion <- function(fit.result){
  ...
  # round(prop.table(tmp$table,2),4) # Results by column (coincide with sens and spec)
...
sigma <- 1 # variance of the error term of the MSM
                 ...
                 .combine=rbind) %dorng%
    {
      ...
      if (method == "Symbolic"){
        ...
        for (j in 1:100){ # Try up to 50 times, just in case of numerical error
          res[[j]] <- try(fitSYM_2ALR_TS_2STATES(symcoordata), silent = TRUE)
          if (!is.character(res[[j]])){
            fitmodel <- res[[j]]
            break
          }
