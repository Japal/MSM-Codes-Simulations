...
## MS model fit to alr coords (using Murcia's implementation)
fitSYM_2ALR_TS_3STATES.breaks <- function(X, width = 0){
  ...
  # Split obs state into steady/break (0 vs 1) (I use "status" to refer to this binary partition)
  obs.status <- c(0,diff(res$State)) # Add 0 at the beginning to mark start of new original state
  # Split pred state (up, steady, down) into steady/break
  pred.status <- apply(res[,-1],1, which.max) # Identify highest probability for each observation (column position)
  ...
  break.at <- which(obs.status==1) # where the actual breaks are
  matches <- rep(0,3)
  
  for(i in 1:3){ # Define matching-up neighbourhood around the three break points and count indicate matches in pred
    if(any(pred.status[(break.at[i]-width):(break.at[i]+width)] == 1)){matches[i] <- 1}
  }
  
  return(matches) # returns only the prediction (break 1, steady 0) for the (neighbourhood of the) obs breaks
}
...
method <- c("Symbolic") 
width <- c(0) # Width of neighbourhood around the break point to check match-up
