...
create.data <- function(n.series , l.series, prob00 = 0.9, prob11 = 0.7, alpha = 1.5, sigma = 1){
  ...
  p00k <- prob00 # Prob inertia state 0
  p11k <- prob11 # Prob inertia state 1
...
make.missing <- function(dat, p){
  
  # dat: including first State column
  # p: p% of time series including missing values
  ...
  rows <- sample(1:nrow(dat),round(p*nrow(dat),0)) # random selection of p% series (rows) with missing 
  perc.cols <- runif(length(rows),0.2,0.6) # Between 20-60% elements of a series containing missing for each row
  num.cols <- round(perc.cols*ncol(dat),0)
  for (i in 1:length(rows)){
    dat[rows[i], 
        sample(ncol(dat),num.cols[i])] <- NA # random selection of cols with imposed missing applied to each row
  }
...
p <- c(0,0.2,0.4,0.6,0.8) # Prop of series including missing
method <- c("Symbolic")
...
set.seed(534534) # Generate same data set for different methods in the otherwise same scenario
...
for (case in 1:ncases){
  ...
  TAB <- foreach(k = 1:nsim,
                ...
                .combine=rbind) %dorng%
      {
        ...
        # Impose missing
        mdata <- make.missing(dat = data, p = p) # data set with missing imposed
        # Fit MSM
        if (method == "Symbolic"){
